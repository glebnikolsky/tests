...
History: PJN / 11-01-2006 CNTServiceControlManager::EnumServices method now uses a void* parameter for its 
         ...
         PJN / 06-11-2019 1. Removed unnecessary return value from CNTServiceControlManager::Attach method
...
void CNTServiceControlManager::Attach(_In_opt_ SC_HANDLE hSCM) noexcept
{
  if (m_hSCM != hSCM)
    Close();
  m_hSCM = hSCM;
...
_Must_inspect_result_ _Return_type_success_(return != 0) BOOL CNTServiceControlManager::QueryLockStatus(_Inout_ CNTServiceControlManagerLockStatus& lockStatus) const
{
...
#pragma warning(suppress: 26477)
  ...
  {
    lockStatus.m_Data.resize(dwBytesNeeded);
...
#pragma warning(suppress: 26429 26490)
    auto pServices = reinterpret_cast<LPENUM_SERVICE_STATUS>(pBuffer.data());
...
#pragma warning(suppress: 26477)
  ...
  if (!bSuccess && (GetLastError() == ERROR_MORE_DATA))
  {
    ...
    pBuffer.resize(dwBytesNeeded);
#pragma warning(suppress: 26490)
    auto pServices = reinterpret_cast<LPENUM_SERVICE_STATUS_PROCESS>(pBuffer.data());
