...
#pragma warning(suppress: 26490)
    return reinterpret_cast<LPQUERY_SERVICE_LOCK_STATUS>(m_Data.data());
  }
...
public:
//typedefs
  using ENUM_SERVICES_PROC = bool (CALLBACK *)(void* pData, const ENUM_SERVICE_STATUS& ss);
  using ENUM_SERVICES_PROC2 = bool (CALLBACK *)(void* pData, const ENUM_SERVICE_STATUS_PROCESS& ssp);
...
//Methods
  ...
  void Attach(_In_opt_ SC_HANDLE hSCM) noexcept;
  ...
  //Get the SCM Status
  _Must_inspect_result_ _Return_type_success_(return != 0) BOOL QueryLockStatus(_Inout_ CNTServiceControlManagerLockStatus& lockStatus) const;

  //Enumerates the specified services
  _Must_inspect_result_ _Return_type_success_(return != 0) BOOL EnumServices(_In_ DWORD dwServiceType, _In_ DWORD dwServiceState, _In_opt_ void* pUserData, _In_ ENUM_SERVICES_PROC pEnumServicesFunc) const;
...
protected:
//typedefs
  using LPREGISTERSERVICECTRLHANDLEREX = SERVICE_STATUS_HANDLE (WINAPI *)(LPCTSTR, LPHANDLER_FUNCTION_EX, LPVOID);
  using LPENUMSERVICESSTATUSEX = BOOL (WINAPI *)(SC_HANDLE, SC_ENUM_TYPE, DWORD, DWORD, LPBYTE, DWORD, LPDWORD, LPDWORD, LPDWORD, LPCTSTR);
