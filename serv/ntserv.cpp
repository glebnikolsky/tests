...
History: PJN / 17-05-1999 1. Fixed a warning when compiled with VC 6.
         ...
         PJN / 02-06-2019 1. Updated copyright details
                          ...
                          3. Replaced all occurrences of ATL::CHeapPtr with std::vector
         PJN / 28-07-2019 1. Fixed some further compiler warnings when using VC 2019 Preview
         PJN / 13-08-2019 1. Fixed some compile problems when _ATL_NO_AUTOMATIC_NAMESPACE is defined
                          2. Fixed some further compiler warnings when using VC 2019 Preview v16.3.0 Preview 2.0
                          3. Fixed a problem in CNTService::GetServiceParametersKey where the bReadOnly parameter was not being used to open some registry keys.
         PJN / 17-09-2019 1. Remove noexcept attribute from a number of CNTService virtual methods
         PJN / 03-11-2019 1. Updated initialization of various structs to use C++ 11 list initialization
         PJN / 06-11-2019 1. Replaced BOOL with bool in a number of places throughout the framework
         PJN / 28-12-2019 1. Fixed various Clang-Tidy static code analysis warnings in the code.
         PJN / 05-01-2020 1. Fixed more Clang-Tidy static code analysis warnings in the code.
         PJN / 26-07-2020 1. Fixed more Clang-Tidy static code analysis warnings in the code.
                          2. Fixed up the logic and return values for the CNTService::GetRegistryStateKey and CNTService::GetServiceDirectory methods now that 
                          there is proper documentation available for these APIs.
...
CNTService::CNTService() noexcept : m_dwServiceType(SERVICE_WIN32_OWN_PROCESS),
...
#pragma warning(suppress: 26477)
  ATLASSERT(m_hStatus == nullptr); //If this ASSERTs, most likely you are calling RegisterCtrlHandler
...
#pragma warning(suppress: 26477)
  ...
  if (lResult == ERROR_SUCCESS)
  {
    ...
    ATL::CRegKey serviceKey;
#pragma warning(suppress: 26477)
    lResult = serviceKey.Create(servicesKey, pszServiceName, REG_NONE, REG_OPTION_NON_VOLATILE, bReadOnly ? KEY_READ : KEY_WRITE | KEY_READ, nullptr);
    if (lResult == ERROR_SUCCESS)
    {
      //Create the parameters key
#pragma warning(suppress: 26477)
      lResult = serviceParametersKey.Create(serviceKey, _T("Parameters"), REG_NONE, REG_OPTION_NON_VOLATILE, bReadOnly ? KEY_READ : KEY_WRITE | KEY_READ, nullptr);
      if ((lResult != ERROR_SUCCESS) && (pLastError != nullptr))
        *pLastError = lResult;
    }
    else
...
_Return_type_success_(return != 0) bool CNTService::GetDependencies(_Out_ std::vector<TCHAR>& mszDependencies)
{
  ...
  //Now form the string
  mszDependencies.resize(0);
  mszDependencies.reserve(nSize);
  for (const auto& dependency : m_sDependencies)
  {
#pragma warning(suppress: 26481 26489)
    mszDependencies.insert(mszDependencies.end(), dependency.c_str(), dependency.c_str() + dependency.length());
#pragma warning(suppress: 26489)
    mszDependencies.push_back(_T('\0'));
  }
#pragma warning(suppress: 26489)
  mszDependencies.push_back(_T('\0'));
...
#pragma warning(suppress: 26486 26489)
    ...
    _tsplitpath_s(sAppPath, sDrive.GetBuffer(_MAX_DRIVE), _MAX_DRIVE, sDir.GetBuffer(_MAX_DIR), _MAX_DIR, sFname.GetBuffer(_MAX_FNAME), _MAX_FNAME, nullptr, 0);
    sDrive.ReleaseBuffer();
    sDir.ReleaseBuffer();
    sFname.ReleaseBuffer();
    _tmakepath_s(sPath.GetBuffer(_MAX_PATH), _MAX_PATH, sDrive, sDir, sFname, _T("DLL"));
    sPath.ReleaseBuffer();
    m_sEventMessageFile = sPath;
  }
...
#pragma warning(suppress: 26812)
  ...
  {
    std::vector<BYTE> sid;
    sid.resize(dwSIDSize);
    std::vector<TCHAR> domain;
    domain.resize(dwDomainNameSize);
    if (!LookupAccountName(nullptr, m_sUserName.c_str(), sid.data(), &dwSIDSize, domain.data(), &dwDomainNameSize, &snu))
    {
      dwError = GetLastError();
    ...
    ATL::CAtlStringW sPrivilege(L"SeServiceLogonRight");
...
#pragma warning(suppress: 26486)
    ntStatus = LsaAddAccountRights(lsaHandle, sid.data(), &userRights, 1);
...
#pragma warning(suppress: 26486 26489)
  ...
  std::vector<TCHAR> mszDependencies;
...
#pragma warning(suppress: 26486 26489)
  ...
  if (!service.Create(manager, m_sServiceName.c_str(), m_sDisplayName.c_str(), SERVICE_CHANGE_CONFIG, m_dwServiceType, m_dwStartType, 
                      ...
                      nullptr, mszDependencies.data(), m_sUserName.length() ? m_sUserName.c_str() : nullptr, 
...
//Member variables
  ...
  ATL::CAtlString* psAppPath; //The full path to this exe
...
#pragma warning(suppress: 26477)
  ...
  auto pInfo = static_cast<_NTSERV_EnumerateInstancesInfo*>(pData);
#pragma warning(suppress: 26477)
...
#pragma warning(suppress: 26477)
  ...
  if (pInfo->pManager->OpenService(ss.lpServiceName, SERVICE_QUERY_CONFIG, service))
  {
    ...
    if (service.QueryConfig(config))
    {
      ...
      ATL::CAtlString sBinaryPath(config.operator LPQUERY_SERVICE_CONFIG()->lpBinaryPathName);
...
{
  for (int i=1; i<__argc; i++) //NOLINT(clang-analyzer-core.CallAndMessage)
  {
#pragma warning(suppress: 26481)
    ATL::CAtlString sParam(__targv[i]);
    bool bFlag = false;
...
#pragma warning(suppress: 26440 26487)
...
_Must_inspect_result_ _Return_type_success_(return == 0) DWORD CNTService::GetRegistryStateKey(_In_ SERVICE_REGISTRY_STATE_TYPE StateType, _In_ DWORD AccessMask, _Out_ HKEY* ServiceStateKey)
{
  ...
  {
    ATLTRACE(_T("CNTService::GetRegistryStateKey, GetServiceRegistryStateKey function is not supported on this OS. You need to be running at least Windows 10 version 1803 to use this function\n"));
    return ERROR_CALL_NOT_IMPLEMENTED;
  }
...
_Must_inspect_result_ _Return_type_success_(return == 0) DWORD CNTService::GetServiceDirectory(_In_ SERVICE_DIRECTORY_TYPE eDirectoryType, _Out_writes_opt_(cchPathBufferLength) PWCHAR lpPathBuffer, _In_ DWORD cchPathBufferLength, _Out_ DWORD* lpcchRequiredBufferLength)
{
  ...
  {
    ATLTRACE(_T("CNTService::GetServiceDirectory, GetServiceDirectory function is not supported on this OS. You need to be running at least Windows 10 version 1809 to use this function\n"));
    return ERROR_CALL_NOT_IMPLEMENTED;
  }
