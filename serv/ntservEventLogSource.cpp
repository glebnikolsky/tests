...
History: PJN / 23-07-2005 1. Provided an overriden version of Report which has 3 string parameters
         ...
         PJN / 06-11-2019 1. Removed unnecessary return value from CNTEventLogSource::Attach method
...
void CNTEventLogSource::Attach(_In_opt_ HANDLE hEventSource) noexcept
{
...
#pragma warning(suppress: 26489)
          ...
          if (!bFoundMyself)
          {
            sources.emplace_back(pszSourceName);
...
#pragma warning(suppress: 26477)
  ...
  if (lResult == ERROR_SUCCESS)
  {
    ...
    if (nChars < 2) //Ensure we can handle an empty MULTI_SZ string
      nChars = 2;
    pBuffer.resize(nChars);

    lResult = key.QueryMultiStringValue(pszEntry, pBuffer.data(), &nChars);
    if (lResult == ERROR_SUCCESS)
    {
      LPCTSTR pszStrings = pBuffer.data();
#pragma warning(suppress: 26477)
...
#pragma warning(suppress: 26481)
    pBuffer.insert(pBuffer.end(), element.c_str(), element.c_str() + element.length());
    pBuffer.push_back(_T('\0'));
  }
  pBuffer.push_back(_T('\0'));

  //Finally write it into the registry
  LSTATUS lResult = key.SetMultiStringValue(pszEntry, pBuffer.data());
