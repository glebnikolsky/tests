...
History: PJN / 09-01-2006 1. When registering the service, the quoting of the path name to use for the service 
         ...
         PJN / 06-11-2019 1. Removed unnecessary return value from CNTScmService::Attach method
         PJN / 26-07-2020 1. Updated the codebase to support GetSharedServiceRegistryStateKey and 
                          GetSharedServiceDirectory APIs
...
CNTScmService::CNTScmService() noexcept : m_hService(nullptr),
                                          ...
                                          m_pfnGetSharedServiceRegistryStateKey(nullptr),
                                          m_pfnGetSharedServiceDirectory(nullptr)
{
...
#pragma warning(suppress: 26490)
    m_pfnGetSharedServiceRegistryStateKey = reinterpret_cast<LPGETSHAREDSERVICEREGISTRYSTATEKEY>(GetProcAddress(m_hSecHost, "GetSharedServiceRegistryStateKey"));
#pragma warning(suppress: 26490)
    m_pfnGetSharedServiceDirectory = reinterpret_cast<LPGETSHAREDSERVICEDIRECTORY>(GetProcAddress(m_hSecHost, "GetSharedServiceDirectory"));
  }
}
...
void CNTScmService::Attach(_In_opt_ SC_HANDLE hService) noexcept
{
...
_Return_type_success_(return != 0) BOOL CNTScmService::QueryConfig(_Inout_ CNTServiceConfig& config) const
{
  //Validate our parameters
#pragma warning(suppress: 26477)
  ATLASSUME(m_hService != nullptr);
  ...
  {
    config.m_Data.resize(dwBytesNeeded);
    DWORD dwSize = 0;
...
_Must_inspect_result_ _Return_type_success_(return != 0) BOOL CNTScmService::QueryObjectSecurity(_In_ SECURITY_INFORMATION dwSecurityInformation, _Inout_ CNTServiceSecurityDescriptor& securityDescriptor) const
{
  //Validate our parameters
#pragma warning(suppress: 26477)
  ATLASSUME(m_hService != nullptr);
  ...
  if (!bSuccess && (GetLastError() == ERROR_INSUFFICIENT_BUFFER))
  {
    securityDescriptor.m_Data.resize(dwBytesNeeded);
    DWORD dwSize = 0;
...
_Must_inspect_result_ _Return_type_success_(return != 0) BOOL CNTScmService::EnumDependents(_In_ DWORD dwServiceState, _In_opt_ void* pUserData, _In_ ENUM_SERVICES_PROC pEnumServicesFunc) const
{
  //Validate our parameters
#pragma warning(suppress: 26477)
  ATLASSUME(m_hService != nullptr);
...
#pragma warning(suppress: 26429 26490)
    auto pServices = reinterpret_cast<const LPENUM_SERVICE_STATUS>(pBuffer.data());

    DWORD dwSize = 0;
#pragma warning(suppress: 26486 26490)
    bSuccess = EnumDependentServices(m_hService, dwServiceState, pServices, dwBytesNeeded, &dwSize, &dwServices);
    if (bSuccess)
    {
      bool bContinue = true;
...
#pragma warning(suppress: 28020)
  ...
  if (!bSuccess && (GetLastError() == ERROR_INSUFFICIENT_BUFFER))
  {
    ...
    description.resize(dwBytesNeeded);
    bSuccess = QueryServiceConfig2(m_hService, SERVICE_CONFIG_DESCRIPTION, description.data(), dwBytesNeeded, &dwBytesNeeded);
    if (bSuccess)
    {
#pragma warning(suppress: 26489 26490)
      LPCTSTR pszDescription = reinterpret_cast<LPSERVICE_DESCRIPTION>(description.data())->lpDescription;
...
_Must_inspect_result_ _Return_type_success_(return != 0) BOOL CNTScmService::QueryFailureActions(_Inout_ CNTServiceFailureActions& actions) const
{
  //Validate our parameters
#pragma warning(suppress: 26477)
  ATLASSUME(m_hService != nullptr);
...
#pragma warning(suppress: 28020)
  ...
  if (!bSuccess && (GetLastError() == ERROR_INSUFFICIENT_BUFFER))
  {
    actions.m_Data.resize(dwBytesNeeded);
    bSuccess = QueryServiceConfig2(m_hService, SERVICE_CONFIG_FAILURE_ACTIONS, actions.m_Data.data(), dwBytesNeeded, &dwBytesNeeded);
  }
...
#pragma warning(suppress: 26477)
  ...
  std::vector<TCHAR> pBuffer;
  pBuffer.reserve(nSize);

  //Now copy the strings into the buffer
  for (const auto& privilege : privileges)
  {
#pragma warning(suppress: 26481)
    pBuffer.insert(pBuffer.end(), privilege.c_str(), privilege.c_str() + privilege.length());
    pBuffer.push_back(_T('\0'));
  }
  pBuffer.push_back(_T('\0'));

  SERVICE_REQUIRED_PRIVILEGES_INFO srpi;
  srpi.pmszRequiredPrivileges = pBuffer.data();
...
#pragma warning(suppress: 28020)
  ...
  if (!bSuccess && (GetLastError() == ERROR_INSUFFICIENT_BUFFER))
  {
    std::vector<BYTE> privilegesData;
    privilegesData.resize(dwBytesNeeded);
    bSuccess = QueryServiceConfig2(m_hService, SERVICE_CONFIG_REQUIRED_PRIVILEGES_INFO, privilegesData.data(), dwBytesNeeded, &dwBytesNeeded);
    if (bSuccess)
    {
#pragma warning(suppress: 26490)
      auto pszStrings = reinterpret_cast<LPTSTR>(privilegesData.data());
#pragma warning(suppress: 26477)
...
#pragma warning(suppress: 26486 26489)
        privileges.emplace_back(pszStrings);
#pragma warning(suppress: 26481 26486)
...
_Must_inspect_result_ _Return_type_success_(return != 0) BOOL CNTScmService::QueryTrigger(_Inout_ CNTServiceTriggerInfo& triggerInfo) const
{
  //Validate our parameters
#pragma warning(suppress: 26477)
  ATLASSUME(m_hService != nullptr);
  ...
  if (!bSuccess && (GetLastError() == ERROR_INSUFFICIENT_BUFFER))
  {
    triggerInfo.m_Data.resize(dwBytesNeeded);
#pragma warning(suppress: 26490)
...
#pragma warning(suppress: 26440 26487 26812)
...
#pragma warning(suppress: 26487 26440 26812)
_Must_inspect_result_ _Return_type_success_(return == 0) DWORD CNTScmService::GetSharedRegistryStateKey(_In_ SERVICE_SHARED_REGISTRY_STATE_TYPE StateType, _In_ DWORD AccessMask, _Out_ HKEY* ServiceStateKey) const
{
  //Validate our parameters
#pragma warning(suppress: 26477)
  ATLASSUME(m_hService != nullptr);
  ...
  //Check to see if the function pointer is available
  if (m_pfnGetSharedServiceRegistryStateKey == nullptr)
  {
    ATLTRACE(_T("CNTService::GetSharedRegistryStateKey, GetSharedServiceRegistryStateKey function is not supported on this OS. You need to be running at least Windows 10 codename '20H2' to use this function\n"));
    return ERROR_CALL_NOT_IMPLEMENTED;
  }

  return m_pfnGetSharedServiceRegistryStateKey(m_hService, StateType, AccessMask, ServiceStateKey);
}

#pragma warning(suppress: 26440 26812)
_Must_inspect_result_ _Return_type_success_(return == 0) DWORD CNTScmService::GetSharedDirectory(_In_ SERVICE_SHARED_DIRECTORY_TYPE DirectoryType, _Out_writes_to_opt_(PathBufferLength, *RequiredBufferLength) PWCHAR PathBuffer, _In_ DWORD PathBufferLength, _Out_ DWORD* RequiredBufferLength) const
{
  //Validate our parameters
#pragma warning(suppress: 26477)
  ATLASSUME(m_hService != nullptr);
  ...
  if (m_pfnGetSharedServiceRegistryStateKey == nullptr)
  {
    ATLTRACE(_T("CNTService::GetSharedDirectory, GetSharedServiceDirectory function is not supported on this OS. You need to be running at least Windows 10 codename '20H2' to use this function\n"));
    return ERROR_CALL_NOT_IMPLEMENTED;
  }

  return m_pfnGetSharedServiceDirectory(m_hService, DirectoryType, PathBuffer, PathBufferLength, RequiredBufferLength);
}
