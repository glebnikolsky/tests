...
Purpose: Various defines which the CNTService class framework requires
...
{
  typedef enum _SC_EVENT_TYPE //NOLINT(modernize-use-using)
  {
    SC_EVENT_DATABASE_CHANGE,
    SC_EVENT_PROPERTY_CHANGE,
    SC_EVENT_STATUS_CHANGE
  } SC_EVENT_TYPE, *PSC_EVENT_TYPE; //NOLINT(modernize-use-using)
}
...
{
  typedef SC_NOTIFICATION_CALLBACK* PSC_NOTIFICATION_CALLBACK; //NOLINT(modernize-use-using)
}

__if_not_exists (PSC_NOTIFICATION_REGISTRATION)
{
  typedef struct _SC_NOTIFICATION_REGISTRATION* PSC_NOTIFICATION_REGISTRATION; //NOLINT(modernize-use-using)
}

__if_not_exists (SERVICE_REGISTRY_STATE_TYPE)
{
  typedef enum SERVICE_REGISTRY_STATE_TYPE //NOLINT(modernize-use-using)
  {
...
{
  typedef enum SERVICE_DIRECTORY_TYPE //NOLINT(modernize-use-using)
  {
...
__if_not_exists (SERVICE_SHARED_REGISTRY_STATE_TYPE)
{
  typedef enum SERVICE_SHARED_REGISTRY_STATE_TYPE //NOLINT(modernize-use-using)
  {
    ServiceSharedRegistryPersistentState = 0
  } SERVICE_SHARED_REGISTRY_STATE_TYPE;
}

__if_not_exists (SERVICE_SHARED_DIRECTORY_TYPE)
{
  typedef enum SERVICE_SHARED_DIRECTORY_TYPE //NOLINT(modernize-use-using)
  {
