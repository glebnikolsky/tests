...
#pragma warning(suppress: 26490)
    return reinterpret_cast<LPQUERY_SERVICE_CONFIG>(m_Data.m_pData);
  }
...
//Methods
  ...
  {
#pragma warning(suppress: 26490)
    return reinterpret_cast<LPSERVICE_FAILURE_ACTIONS>(m_Data.m_pData);
  }
...
//Methods
  ...
  {
#pragma warning(suppress: 26490)
    return reinterpret_cast<PSERVICE_TRIGGER_INFO>(m_Data.m_pData);
  }
...
public:
//typedefs
  typedef BOOL (CALLBACK* ENUM_SERVICES_PROC)(_In_opt_ void* pUserData, _In_ const ENUM_SERVICE_STATUS& ss);
...
//Methods
  ...
  _Return_type_success_(return != 0) BOOL Attach(_In_opt_ SC_HANDLE hService) noexcept;
  ...
  _Return_type_success_(return != 0) BOOL Stop() const noexcept; //Ask the service to stop
  _Return_type_success_(return != 0) BOOL Pause() const noexcept; //Ask the service to pause
  _Return_type_success_(return != 0) BOOL Continue() const noexcept; //Ask the service to continue
  _Return_type_success_(return != 0) BOOL Interrogate() const noexcept; //Ask the service to update its status to the SCM
  ...
  _Return_type_success_(return != 0) BOOL QueryConfig(_Inout_ CNTServiceConfig& config) const noexcept;
  ...
  _Return_type_success_(return != 0) BOOL Delete() const noexcept;

  //Enumerate the services that this service depends upon
  _Must_inspect_result_ _Return_type_success_(return != 0) BOOL EnumDependents(_In_ DWORD dwServiceState, _In_opt_ void* pUserData, _In_ ENUM_SERVICES_PROC pEnumServicesFunc) const noexcept;

  //Get the security information associated with this service
  _Must_inspect_result_ _Return_type_success_(return != 0) BOOL QueryObjectSecurity(_In_ SECURITY_INFORMATION dwSecurityInformation, _Inout_ CNTServiceSecurityDescriptor& securityDescriptor) const noexcept;
  ...
  _Must_inspect_result_ _Return_type_success_(return != 0) BOOL QueryFailureActions(_Inout_ CNTServiceFailureActions& actions) const noexcept;
  ...
  _Return_type_success_(return != 0) BOOL  ChangeRequiredPrivileges(_In_ const CNTServiceStringArray& priviledges) noexcept;
  ...
  _Must_inspect_result_ _Return_type_success_(return != 0) BOOL QueryTrigger(_Inout_ CNTServiceTriggerInfo& triggerInfo) const noexcept;
