// RepGrid.h : interface of the RepGrid class
//
/////////////////////////////////////////////////////////////////////////////
#include "ugctrl.h"

class RepGrid:public CUGCtrl
{
public:
	RepGrid();
	~RepGrid();
...
private:
	struct ColInf{
		ColInf(const TCHAR*name,int algn, bool v = true, int len=50):header( std::move(name) ), align( algn ), visible(v), length(len) {}
		CString header;
		int		align;
		int		length;
		bool    visible;
	};
	std::vector<ColInf> column_;
